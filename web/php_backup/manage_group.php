...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_member') {
        ...
        if (empty($memberName)) {
            ...
            if ($stmt->execute([$_SESSION['group_id'], $memberName])) {
                $msg = 'added';
            } else {
                $msg = 'error';
            }
        ...
        if ($stmt->execute([$_POST['member_id'], $_SESSION['group_id']])) {
            $msg = 'removed';
        }
    }
}
...
$membersStmt = $conn->prepare("SELECT member_id, member_name FROM group_members WHERE group_id = ? ORDER BY member_name");
$membersStmt->execute([$_SESSION['group_id']]);
$memberList = $membersStmt->fetchAll();
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
    <div class="container">
        ...
        <div class="card">
            <h3>Current Members (<?= count($memberList) ?>)</h3>
