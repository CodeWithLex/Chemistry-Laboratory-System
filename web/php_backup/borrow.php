...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['request_apparatus'])) {
        ...
        if (!empty($apparatusName)) {
            ...
            if ($stmt->execute([$_SESSION['group_id'], $apparatusName, $reason])) {
                $msg = 'apparatus_requested';
            }
        }
    } else {
        // Normal borrow request
        // Check for duplicate pending/approved request before inserting
        $dupCheck = $conn->prepare("SELECT COUNT(*) AS cnt FROM requests WHERE group_id = ? AND apparatus_id = ? AND status IN ('Pending','Approved')");
        $dupCheck->execute([$_SESSION['group_id'], $_POST['apparatus_id']]);
        $dupRow = $dupCheck->fetch();

        if ($dupRow['cnt'] > 0) {
            $msg = 'duplicate';
        } else {
            // Validate quantity against real_available
            ...
            $availStmt->execute([$_POST['apparatus_id'], $_POST['apparatus_id']]);
...
// Feature 2: Show apparatus as long as current_quantity > 0
// Also exclude apparatus that the group already has a pending/approved request for
$apparatusStmt = $conn->prepare(
...
$apparatusStmt->execute();
$apparatusList = $apparatusStmt->fetchAll();
...
$pendingByOthersStmt->execute([$_SESSION['group_id']]);
$pendingByOthersRows = $pendingByOthersStmt->fetchAll();

$pendingByOthers = [];
foreach ($pendingByOthersRows as $row) {
    $aid = (int)$row['apparatus_id'];
    $pendingByOthers[$aid][] = $row;
}

// All requests by this group
$requestsStmt = $conn->prepare("SELECT r.*, a.item_name FROM requests r JOIN apparatus a ON r.apparatus_id = a.apparatus_id WHERE r.group_id = ? ORDER BY created_at DESC");
$requestsStmt->execute([$_SESSION['group_id']]);
$myRequests = $requestsStmt->fetchAll();

// Currently Borrowing: approved items
$borrowingStmt = $conn->prepare("SELECT r.*, a.item_name FROM requests r JOIN apparatus a ON r.apparatus_id = a.apparatus_id WHERE r.group_id = ? AND r.status = 'Approved' ORDER BY r.updated_at DESC");
$borrowingStmt->execute([$_SESSION['group_id']]);
$myBorrowing = $borrowingStmt->fetchAll();

// All apparatus names (for the pending-by-others section)
$nameStmt = $conn->query("SELECT apparatus_id, item_name FROM apparatus");
$allApparatusNames = [];
...
<body>
    <div class="container">
        ...
        <?php if (!empty($myBorrowing)): ?>
        <div class="card">
            <h3>📦 Currently Borrowing</h3>
            <?php foreach ($myBorrowing as $b): ?>
            ...
            <form method="POST">
                <div class="form-group">
                    ...
                    <select name="apparatus_id" required>
                        ...
                        <?php foreach ($apparatusList as $a): ?>
                            <?php
